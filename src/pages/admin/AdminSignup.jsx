...
export default function AdminSignup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    const res = await register(fullname, email, password, 'admin');
    if (res.success) {
        ...
        <div className="max-w-[420px] w-full flex flex-col justify-center py-8">
          ...
          <div className="text-center mb-6">
            ...
            <p className="font-body-md text-sm text-secondary">
              Create an administrative account to manage NexaCart.
            </p>
          </div>
          ...
          <p className="mt-6 text-center font-body-md text-secondary">Already have an account? <span onClick={() => navigate("/admin/login")} className="text-primary font-semibold hover:underline cursor-pointer">Sign in here</span></p>
        </div>
      </section>
      ...
      <section className="hidden md:block w-1/2 h-full relative overflow-hidden bg-primary-container">
        ...
        <div className="absolute bottom-12 left-12 right-12 glass-effect p-8 rounded-xl z-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-label-sm text-label-sm text-primary uppercase tracking-widest font-bold">Enterprise Command</span>
          </div>
          <h3 className="font-headline-md text-xl font-bold text-primary mb-2">
            Unmatched Platform Scaling
          </h3>
          <p className="font-body-md text-sm text-secondary">
            Experience the precision of the NexaCart ecosystem, designed to handle billions in volume with architectural stability and millisecond response times.
          </p>
        </div>
      </section>
    </main>
