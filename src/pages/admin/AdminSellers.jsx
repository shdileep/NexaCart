...
export default function AdminSellers() {
  ...
  const handleBlockSeller = (sellerId, currentBlocked) => {
    const allUsers = JSON.parse(localStorage.getItem('mock_users')) || [];
    const user = allUsers.find(u => u.id === sellerId);
    if (user) {
      user.isBlocked = !currentBlocked;
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-surface-container">
                  {filteredSellers.map((seller) => {
                    ...
                    const totalRevenue = sellerProds.reduce((sum, p) => sum + (p.price * p.sales), 0);
                    const isVerified = !!seller.phone;
                    const computedStatus = seller.isBlocked ? 'Blocked' : (isVerified ? 'Verified' : 'Pending');
            ...
            <div className="p-6 overflow-y-auto flex-grow">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-surface-container-low">
                  {products.filter(p => p.sellerId === selectedSeller.id).map((prod) => {
                    const prodRevenue = prod.price * prod.sales;
                    return (
                      <tr key={prod.id} className="hover:bg-surface-bright">
                        ...
                        <td className="px-4 py-3 text-sm font-bold text-primary">₹{prod.price.toLocaleString()}</td>
