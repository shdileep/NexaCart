...
export default function SellerSignup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    const res = await register(fullname, email, password, 'seller', { businessName: businessname });
    if (res.success) {
        ...
        <div className="max-w-[420px] w-full flex flex-col justify-center py-8">
          ...
          <div className="text-center mb-6">
            ...
            <p className="font-body-md text-sm text-secondary">
              Create an enterprise account to sell products.
            </p>
          </div>
          ...
          <div className="space-y-1">
            <label className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider block" htmlFor="fullname">Full Name</label>
            <input 
          ...
          <div className="space-y-1">
            <label className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider block" htmlFor="businessname">Business Name</label>
            <input 
              className="w-full px-4 py-3 rounded-lg border border-outline-variant bg-white text-on-surface focus:ring-0 focus:border-primary transition-all font-body-md" 
              id="businessname" 
              placeholder="Lumina Tech Systems" 
              required 
              type="text" 
              value={businessname}
          ...
          <p className="mt-6 text-center font-body-md text-secondary">Already have an account? <span onClick={() => navigate("/seller/login")} className="text-primary font-semibold hover:underline cursor-pointer">Sign in here</span></p>
        </div>
      </section>
      ...
      <section className="hidden md:block w-1/2 h-full relative overflow-hidden bg-primary-container">
        ...
        <div className="absolute bottom-12 left-12 right-12 glass-effect p-8 rounded-xl z-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-label-sm text-label-sm text-primary uppercase tracking-widest font-bold">Merchant Operations</span>
          </div>
          <h3 className="font-headline-md text-xl font-bold text-primary mb-2">
            Unified Global Supply Chain
          </h3>
          <p className="font-body-md text-sm text-secondary">
            Connect directly to logistics networks, automate inventory management, and scale your merchant enterprise with state-of-the-art tools.
          </p>
        </div>
      </section>
    </main>
